...
def apply_kl_penalty(data: DataProto, kl_ctrl: KLController, kl_penalty="kl", global_step=1, kl_scaling="none"):
    ...
    # Compute scaling default_factory
    ...
    print(f"Step {global_step}, scale={scale}, kl_coef={kl_ctrl.kl_coef}", flush=True)

    current_kl = torch.mean(VF.masked_mean(kld, mask=response_mask, dim=-1)).item()
    metrics = {"actor/kl_penalty": current_kl, "actor/kl_coef": kl_ctrl.kl_coef, "scale": scale}
            ...
            with timer("step", timing_raw):
                ...
                with timer("adv", timing_raw):
                    ...
                    if not self.config.algorithm.use_kl_loss and self.use_reference_policy:
                        ...
                        batch, kl_metrics = apply_kl_penalty(batch, self.kl_ctrl, self.config.algorithm.kl_penalty, global_step=self.global_step, kl_scaling=self.config.algorithm.kl_scaling)
